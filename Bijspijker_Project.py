...
def show_application():
    ...
    frame_start = Frame(master=root, pady=30, padx=30,bg='yellow')
    ...
    btn_show_departures = Button(master=frame_start, text='Toon Vertrektijden', command=frame_departures.tkraise,bg='blue')
...
def create_departures_frame(root, frame_previous):
    ...
    button1 = Button(master=frame_departures, text='Vertrektijden laden', command=lambda: show_departures(text, entry1.get()), bg='blue')
...
def show_departures(txt_output, departure):
    ...
    txt_output.insert(INSERT, departure)
